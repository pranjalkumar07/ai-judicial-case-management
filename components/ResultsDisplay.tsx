...
import { CheckCircle, AlertTriangle, Info, Globe, Users } from 'lucide-react';
...
const InfoCard: React.FC<{ title: string; value: string; urgency?: string }> = ({ title, value, urgency }) => {
  const getUrgencyStyles = () => {
    ...
    switch (urgency.toLowerCase()) {
      case 'high':
        return { icon: <AlertTriangle className="w-5 h-5 mr-1.5" />, color: 'text-red-500' };
      case 'medium':
        return { icon: <AlertTriangle className="w-5 h-5 mr-1.5" />, color: 'text-amber-500' };
      case 'low':
        return { icon: <Info className="w-5 h-5 mr-1.5" />, color: 'text-blue-500' };
      default:
        ...
        {showLawyers && (
            <div className="mt-4">
                ...
                <div className="bg-blue-500/10 border-l-4 border-blue-500 text-blue-800 dark:text-blue-300 p-4 rounded-r-md mb-4" role="alert">
                  <p className="font-bold flex items-center gap-2"><Info className="w-5 h-5"/>{t.results.importantNote}</p>
