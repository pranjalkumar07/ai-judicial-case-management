...
import { Mail, Feather, Gavel } from 'lucide-react';
...
const NotificationIcon: React.FC<{ icon: AppNotification['icon'] }> = ({ icon }) => {
    ...
    switch(icon) {
        case 'case': return <Mail className={`${baseClass} text-blue-500`} />;
        case 'draft': return <Feather className={`${baseClass} text-green-500`} />;
        case 'system': return <Gavel className={`${baseClass} text-slate-500`} />;
    }
};
